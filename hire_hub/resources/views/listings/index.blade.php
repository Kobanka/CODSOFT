<x-app-layout>
    <x-hero :listings="$listings" :tags="$tags"></x-hero>
    {{-- Jobs Listing Section --}}
    <section class="container mx-auto py-12 px-5 bg-slate-100">
        <div class="mb-12">
            <h2 class="text-2xl font-bold text-ClearBlue text-center md:text-3xl">Latest jobs ({{ $listings->count() }})</h2>
        ...
        <div class="-my-6">                
                    ...
                    <div class="flex md:flex-row flex-col gap-x-8 mt-4 md:mt-0">
                        <div class="md:flex-grow md:mr-8 flex items-center gap-x-2 justify-start">
                            @foreach($listing->tags as $tag)
                               <span class="inline-block tracking-wide text-xs font-medium title-font py-0.5 px-1.5 border border-blue-600 uppercase rounded-lg {{ $tag->slug == request()->get('tag') ? 'bg-ClearBlue text-white' : 'text-ClearBlue bg-gray-100' }}">
                                   {{ $tag->name }}
                               </span>
                            @endforeach
                        </div>
            ...
            @empty
                <p class="text-xl text-center text-gray-700">No Jobs Found !!!</p>
            @endforelse
        </div>
    </section>
</x-app-layout>